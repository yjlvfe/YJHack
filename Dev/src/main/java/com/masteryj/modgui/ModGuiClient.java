...
public final class ModGuiClient implements ClientModInitializer {
   ...
   private static final class AutoLeftScreen extends YjScreen {
      ...
      protected void init() {
         ...
         y += YjTheme.ROW + 4;
         this.addSlider(x, y, w, "Min CPS", 1, 30, this.cfg.minCps, true, v -> this.cfg.minCps = (int) Math.round(v));
         addHelp(x, y, w, YjTheme.CTRL_H, "Minimum clicks per second.");
         y += YjTheme.ROW + 2;
         this.addSlider(x, y, w, "Max CPS", 1, 30, this.cfg.maxCps, true, v -> this.cfg.maxCps = (int) Math.round(v));
